...
class _BoardState extends State<BoardCanva> {
  ...
  void _updateSelectionLine(dynamic startCell, int dr, int dc, int steps) {
  ...
  int currentLength = widget.board.selectedCells.length;
  ...
  if (targetLength > currentLength) {
    ...
    for (int i = currentLength; i < targetLength; i++) {
      ...
      if (nextR >= 0 && nextR < widget.board.row && 
          nextC >= 0 && nextC < widget.board.col) {
        
        // AQUÍ ESTÁ LA INTEGRACIÓN:
        // Llamamos a tu método existente. Él se encarga de notifyListeners, etc.
        widget.board.selectCell(nextR, nextC);
      }
    }
  } 
  ...
  // CASO B: Retrocediendo (El usuario encoge la selección)
    ...
    for (int k = 0; k < itemsToRemove; k++) {
      ...
      if (widget.board.selectedCells.isNotEmpty) {
        // Opción 1: Manipulación directa (si tu clase Board lo permite)
        // final removed = widget.board.selectedCells.removeLast();
        // removed.isSelected = false; // Si necesitas apagar flags manualmente
        
        // Opción 2 (Recomendada): Crear un método en tu Board
        // widget.board.removeLastSelection();
      }
    }
    // Importante: Si modificaste la lista directamente aquí (Opción 1), 
    // necesitas notificar al final.
    // widget.board.notifyListeners(); 
  }
}

  List<int>? _getCellFromPosition(Offset localPosition) {
    final size = gamePainter.lastSize;
    if (size == null) return null;
    final cellW = size.width / widget.board.col;
    final cellH = size.height / widget.board.row;
    int c = (localPosition.dx / cellW).floor();
    int r = (localPosition.dy / cellH).floor();
    if (r < 0 || r >= widget.board.row || c < 0 || c >= widget.board.col) return null;
    return [r, c];
  }

  void onPanStart(DragStartDetails details) {
    widget.board.selectedCells.clear();
    final cell = _getCellFromPosition(details.localPosition);
    if (cell != null) {
      final r = cell[0];
      final c = cell[1];
      widget.board.selectCell(r, c);
    }
  }

  void onPanUpdate(DragUpdateDetails details) {
  // 1. Validaciones iniciales
  if (widget.board.selectedCells.isEmpty) return;

  // Obtenemos la celda de INICIO (el ancla)
  final startCell = widget.board.selectedCells.first;
  ...
  final double cellWidth = painter.cellWidth; // O calculate: size.width / cols
  final double cellHeight = painter.cellHeight;
  ...
  if (widget.board.selectedCells.length > 1) {
    final secondCell = widget.board.selectedCells[1];
  ...
  _updateSelectionLine(startCell, dirRow, dirCol, steps);
}

  void onPanEnd(DragEndDetails details) {
    // Process selected word here if needed
    try{
    widget.board.foundWord();
    } catch (e) {
      // ignore: avoid_print
      print(e);
    }
